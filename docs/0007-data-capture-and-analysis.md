...
> The smallest backend that lets a mobile website capture every tap and stroke from the games, turn them into per-day features against a personal baseline, and show the family "today vs usual". One Node process, one SQLite file, no accounts for the person.
...
- Accounts, passwords, or a real identity provider for the family.
...
- A tester can set up a household, get both links, and use them, in under
  three minutes.
...
1. Caregiver opens the setup page, enters the person's first name, the
   family names and photos (for 0004/0005), and their own name. Gets two
   links: one for the person's phone ("Add to home screen"), one for the family.
2. The person opens their link. The page starts a session, plays the games,
   ...
   and send later.
3. The family member opens their link and sees the last seven days and one
   sentence for today. On a flagged day, a single question is asked: "Has
   Margaret seemed more confused than usual today?" Yes / No.

### 5.2 Screens and states

| Screen | Purpose | Primary action | States (empty / loading / error / success) |
|--------|---------|----------------|--------------------------------------------|
| Setup | create a household | Make the links | empty; saved (shows both links, copy buttons); name missing |
...
| Family view | today vs usual | none (or answer the question) | no data yet ("Margaret hasn't signed yet"); baseline building ("3 of 6 days"); normal; different from usual; no visit today |
...
time for the day, client deltas for within-session timing); a link shared
with the wrong person (revoke and reissue from setup).
...
| links | token, household_id, role (person, family, setup), revoked_at | identity is the link |
...
| people | id, household_id, name, relation, photo_path, words JSON, clues JSON | content for 0003–0005 |
...
All routes take the link token as a bearer header; the token decides the
household and the role.

| Route | Role | Body / response |
|---|---|---|
| `POST /api/households` | setup (open in prototype) | `{personName, tz, family: [...]}` → `{personLink, familyLink}` |
...
| `GET /api/family/summary?days=7` | family | `[{day, visited, sentence, flags, signaturePath, composite}]`, baseline status, pending question |
| `POST /api/family/informant` | family | `{day, answer}` |
| `POST /api/links/revoke` | setup | `{token}` |
...
| Identity | capability links, no accounts | magic-link email, passwords, OAuth | the person never types; family links are shareable by the caregiver; revocation is enough for a prototype |
...
| M2 | server: households, links, sessions, events, family summary; SQLite | prototype hour | not started |
| M3 | client capture queue + demo mode; family view | prototype hour | not started |
...
|----------|-------|-----------|
| Is a capability link acceptable for the family side, or do they need an email magic link? | @cpluntke | before build |
